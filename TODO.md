...
- [ ] **ふぁぼ/RT が実際に反映されない**: デバッグログ追加 + Bluesky CID パース修正済み。X の queryId が古い可能性あり → ログ確認後に対応
...
- [ ] **ユーザープロフィール + 個人TL**: プロフィール画面の骨格は実装済み。個人タイムライン取得が未実装
- [ ] **フォロー/アンフォロー**: プロフィール画面からフォロー操作
- [ ] **投稿作成**: テキスト投稿の作成・送信機能
- [ ] **RT の表示形式改善**: 現状すべて引用形式で表示。通常 RT と引用 RT を区別して表示する
